...
```text
เขาให้หาว่าจำนวนรอบที่คนในวงเปิดกล่อง มีกี่ครั้งโดยที่ยังไม่โดนในมือโจรหรือหยุดที่คนที่หนึ่งพอดี
โดยหลักการวน จะส่งต่อแบบผ่านกี่คนแล้วหยุดที่ตรงไหน (1 + k) ไปเรื่อยๆจนหยุดที่คนที่หนึ่งหรือคนขโมย
```
...
```text
รับค่าแล้ว ทำจำนวนคนให้เป็นลิสต์จำนวนตาม N แล้วพอจะเริ่มนับให้ใช้ for loop นับขั้นตามk เรื่อยๆ แล้วไล่ลำดับตำแหน่งในลิสต์ ว่าตรงกับ คนที่1 หรือ ขโมยไหม ถ้ายังไม่ตรงให้นับจำนวนครั้งไว้ ถ้าตรงให้ออกลูบออกมา และปริ้นค่า
```
...
```text
รับค่า ใช้ while loop ทีแรกเช็คก่อนเลยว่าถ้าค่า T = 1 ให้ออกจากลูบเลย 
ถ้าเงื่อนไขนั้นไม่ใช้ ให้คำนวนให้ค่า k วนลูบอยู่ในจำนวนคนที่มี สูตรคือ จำนวนปัจจุบัน บวกนับขั้น หารเอาเศษด้วยจำนวนคน
จากนั้นดูว่าหาเสร็จอยู่ที่ตำแหน่งอะไร อันแรกถ้าตำแหน่งอยู่ที่ขโมยให้ออกลูบ กับอันสองถ้าตำแหน่งอยู่ที่คนที่หนึ่ง  ถ้ายังไม่ออกลูบให้นับจำนวนรอบที่ของขวัญวนรอบ

เอาท์พุดจำนวนรอบที่ของขวัญวนรอบ
...
```text
ทดสอบว่าถ้าเริ่ม T = 1 จะเข้าเงื่อนไขถูกไหม
```
...
```text
ช่วยบอกว่า ใช้สูตรอื่นที่ไม่ใช้ลิสต์ได้ และบอกว่าข้อนี้้ใช้ while loop
```

คุณยังทำอะไรด้วยตนเอง

```text
พยายามเขียนด้วยตัวเอง
```
