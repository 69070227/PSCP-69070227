...
```text
หาว่า ณ ช่วงเวลาหนึ่งมีร้านเปิดอยู่กี่ร้าน
```
...
```text
คือการตั้งเวลาไว้ 1441 แล้วกำหนดไว้ว่า ช่วงเวลานี้ให้เริ่มเป็น +1 แล้ว ท้ายที่เวลาปิดให้ -1 เพื่อเป็นการกำหนดช่วง
จากนั้นให้ใช้ for loop อีกอันคือรันบวกช่วงเวลา ต่อไปรันตามเวลาทีละตำแหน่ง เริ่มที่ 0 แล้วบวกค่าการเปลี่ยนแปลงของแต่ละตำแหน่งในลิสต์ไปเรื่อย ๆ จนเจอ 600 ที่มีค่าบวกหนึ่ง 601 ก็จะมีค่าเป็น 1 ในลิสต์ ต่อไปจนเจอตำแหน่ง 660 ที่มีค่าเป็น -1 ช่วงเวลาก็จบที่จุดนั้น
...
```text
เริ่มจากสร้างลิสต์ time จำนวน 1441 ช่อง เพื่อเก็บข้อมูลของแต่ละนาทีตั้งแต่ 0 ถึง 1440 จากนั้นกำหนดเวลาเริ่มเปิดของแต่ละร้านเป็น +1 และเวลาปิดเป็น -1 เพื่อบอกว่าจำนวนร้านที่เปิดอยู่มีการเปลี่ยนแปลงตรงเวลาไหน
จากนั้นใช้ for loop รันตามเวลาทีละตำแหน่ง โดยให้ current เริ่มที่ 0 แล้วนำค่าการเปลี่ยนแปลงในแต่ละตำแหน่งมาบวกสะสม เช่น เมื่อถึงเวลา 600 ซึ่งมีค่า +1 ค่า current จะกลายเป็น 1 และเวลาต่อ ๆ ไปจะยังมีค่าเป็น 1 จนถึงเวลา 660 ซึ่งมีค่า -1 ทำให้ current ลดลง แสดงว่าร้านปิดแล้ว และนำค่า current ที่ได้ไปเก็บไว้ใน time[i]
สุดท้ายรับเวลาที่ต้องการตรวจสอบ แล้วใช้ for loop ตามจำนวน check เพื่อดูแต่ละเวลาที่โจทย์กำหนด โดยนำเวลานั้นมาเป็น index ของลิสต์ time เพื่อดูว่าขณะนั้นมีร้านเปิดอยู่กี่ร้าน แล้วแสดงผลออกมา
```
