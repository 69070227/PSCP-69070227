...
| OJ submission ID, if submitted | https://ijudge.it.kmitl.ac.th/submissions/575594/overview |
| OJ status | Pass |
...
```text
ใช้สอน def ว่ามันใช้หลายๆอันเชื่อมกันยังไงอธิบายการทำงานของ def, return, for และการวนลูปทีละขั้น โดย AI ไม่ได้ให้เฉลยของโจทย์ทันที แต่ใช้การถามคำถามและให้แบบฝึกหัด เช่น double(), is_even(), is_vowel() เพื่อให้เข้าใจการใช้ฟังก์ชันก่อน แล้วจึงนำความรู้มาประยุกต์กับโจทย์
```
...
```text
ให้สังเกตว่า จริงๆข้อนี้ไม่จำเป็นต้องใช้ def ก้ได้
```
...
```text
สังเกตว่า จริงๆข้อนี้ไม่จำเป็นต้องใช้ def ก้ได้ และปรับเปลี่ยนโค้ดที่ไม่ต้องใช้ def เลย
```
...
```text
ได้เรียนรู้ว่าควรแบ่งโจทย์ออกเป็นส่วนย่อย เช่น การตรวจสอบว่าเป็นสระหรือไม่ การนับจำนวน และการแสดงผล ทำให้สามารถแก้ปัญหาได้ง่ายขึ้น นอกจากนี้ยังเข้าใจการใช้ def, return, for และการใช้ list สำหรับเก็บจำนวนสระแต่ละตัว
```
