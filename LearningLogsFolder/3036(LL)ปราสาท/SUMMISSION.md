...
```text
https://ijudge.it.kmitl.ac.th/submissions/558580/overview
```
...
```text
เขาให้หาว่าถ้าอยู่ห้องที่มีเลขนี้ จะไปถึงจุดหมายเลขหนึ่งได้น้อยสุดยังไง กี่วิธี
```
...
```text
จากรูปพีระมิดจะสังเกตได้ว่า  จะมีสามเหลี่ยมขึ้นและลง สามเหลี่ยมลงจะเป็นคู่ถ้าหากนับตัวข้างหน้าจะเป็นเลขคี่ ส่วนสามเหลี่ยมหงายจะเป็นคี่นับข้างหน้าจะเป็นคู่
จึงคิดว่า จะให้คำนวนไปที่ละขั้นๆ จากบนสุด รับค่ามา ถ้าเลขยังเยอะกว่าตัวท้าย ให้นับชั้นไปเรื่อยๆ แล้วหาว่าเป็นเลขคู่เลขคี่
```
...
```text
เหมือนแผนที่คิด แต่เพิ่มมา คือรับค่าเลขห้องที่อยู่(ตัวแปร X) จากนั้นตั้งค่าให้เราอยู่ชั้นแรกก่อน ต่อไปค่อยคำนวนว่า ในขณะที่ชั้นที่เราอยู่(1,2,3,4,5)พอนำไปกำลังสองแล้วยังน้อยกว่าจำนวนที่เราใส่เข้าไป(ตัวแปร X) ให้เพิ่มเลขชั้นขึ้นมาหนึ่งชั้น(floor +=1) 
ส่วน far ขั้นตอนนี้คือหาค่าว่า จำนวนห้องที่อยู่หน้าเลข(ตัวแปร X) มีค่าเท่าไหร่
room ใช้หาว่าในชั้นนั้นมีกี่ห้อง
ขั้นตอนสุดท้ายหาค่าว่าได้กี่วิธี คือเอาค่า far มาหารสองว่าลงตัวไหมเหลือเท่าไหร่ ถ้าลงตัวแสดงว่าเป็นเลขคู่และให้room - 1  ถ้าไม่ลงตัวแสดงว่าเป็นเลขคู่และให้room - 2 เพราะจากรูปพีระมิดจะสังเกตได้ว่า  จะมีสามเหลี่ยมขึ้นและลง สามเหลี่ยมลงจะเป็นคู่ถ้าหากนับตัวข้างหน้าจะเป็นเลขคี่ ส่วนสามเหลี่ยมหงายจะเป็นคี่นับข้างหน้าจะเป็นคู่
```
...
```text
ทดสอบโค้ดด้วยเลขอื่นนอกเหนือจากSample Testcases
```
...
```text
อธิบายโจทย์และแนวคิดว่าโจทย์ต้องการอะไรคิดยังไง
```

คุณยังทำอะไรด้วยตนเอง

```text
เข้าใจด้วยตนเอง ลองใช้หลายๆสูตร
```
