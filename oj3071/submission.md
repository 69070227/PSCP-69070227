...
```text
https://ijudge.it.kmitl.ac.th/submissions/577759/overview
```
...
```text
โจทย์ให้หาว่า ในช่วงเอถึงบี[a,b] ใช้ ดี(d) หารแล้วได้เศษ อาร์(r) ได้กี่ตัว
...
```text
Step 1: รับค่ามา
Step 2: ใช้ for loop วนตัวเลขที่เริ่มตั้งแต่เอ ถึง(บี)
Step 3: ในลูบใส่เงื่อนไขว่า ถ้าเลขปัจจุบัน(x) หารเอาเศษกับ ตัวหาร(d) แล้วเศษที่ได้ตรงกับ (r) ให้ count +=1
Step 4: print(count)

ก่อนถามเพื่อน ฉันรู้อยู่แล้วว่าควรต้องคำนวนแค่ในช่วงเอบี
```
...
```text
เหมือนแผนแรก แต่แก้ตรงที่ for loop ที่แรกใช้ for loop x in range(A_start,B_stop):
มันไม่ถูก เพราะมันหยุดก่อนบี(สมมติถ้าบี = 10 มันจะมีช่วงแค่1ถึง9)
เลยเปลี่ยนเป็น for x in range(A_start,B_stop + 1):
```
...
```text
เขาช่วยใบ้ว่า ข้อนี้ใช้ for loop ให้คำนวนในช่วง
```

คุณยังทำอะไรด้วยตนเอง

```text
เข้าใจและเขียนโค้ดด้วยตนเอง
```
